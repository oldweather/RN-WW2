...
unlink(
    "$Data_dir_base/$Variable/5x5/$Opname.pp",
    "$Data_dir_base/$Variable/5x5/nobs_$Opname.pp",
    "$Data_dir_base/$Variable/5x5/sd_$Opname.pp"
);
...
for ( my $Year = 1938 ; $Year <= 1947 ; $Year++ ) {
    for ( my $Month = 1 ; $Month <= 12 ; $Month++ ) {
        ...
        system(
            "$FindBin::Bin/redate.perl < grid.pp >> $Data_dir_base/$Variable/5x5/$Opname.pp"
          ) == 0
          or die "Grid copy failed";
        system(
            "$FindBin::Bin/redate.perl < nobs.pp >> $Data_dir_base/$Variable/5x5/nobs_$Opname.pp"
          ) == 0
          or die "Nobs copy failed";
        system(
            "$FindBin::Bin/redate.perl < SD.pp >> $Data_dir_base/$Variable/5x5/sd_$Opname.pp"
          ) == 0
